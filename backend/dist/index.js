...
const TEAMS_FILE = path_1.default.join(__dirname, 'teams.json');
function getTeamsData() {
    try {
        if (!fs_1.default.existsSync(TEAMS_FILE)) {
            return {};
        }
        const content = fs_1.default.readFileSync(TEAMS_FILE, 'utf-8');
...
function saveTeamsData(data) {
    try {
        fs_1.default.writeFileSync(TEAMS_FILE, JSON.stringify(data, null, 2), 'utf-8');
    }
    catch (err) {
...
app.post('/api/colleges/validate', async (req, res) => {
    ...
    try {
        ...
        const { data: dbData, error: dbError } = await supabase_1.supabaseAdmin
            .from('colleges')
...
app.post('/api/judges', async (req, res) => {
    ...
    try {
        ...
        const { data: existingJudge } = await supabase_1.supabaseAdmin
            .from('judges')
            .select('id')
            .eq('email', email)
            .maybeSingle();
        ...
        const { data: collegeData, error: collegeError } = await supabase_1.supabaseAdmin
            .from('colleges')
            .select('name')
            .eq('id', college_id)
            .single();
            ...
            if (authError.status === 422 || authError.message.includes('email_exists') || authError.code === 'email_exists') {
                ...
                if (usersData && usersData.users) {
                    ...
                    if (existingUser) {
                        ...
                        await supabase_1.supabaseAdmin.auth.admin.updateUserById(userId, { password: tempPassword });
                    }
                    else {
...
app.put('/api/judges/:id', async (req, res) => {
    ...
    try {
        ...
        const { error: authError } = await supabase_1.supabaseAdmin.auth.admin.updateUserById(id, { email });
        if (authError) {
...
app.post('/api/jury/login', async (req, res) => {
    ...
    try {
        ...
        const { data: judgeData, error: dbError } = await supabase_1.supabaseAdmin
            .from('judges')
            .select('*')
            .eq('id', data.user.id)
            .single();
...
app.post('/api/student/send-otp', async (req, res) => {
    ...
    try {
        ...
        if (round1Status === 'closed') {
            const isLeader = student.teams && (Array.isArray(student.teams) ? student.teams[0]?.leader_id === student.id : student.teams?.leader_id === student.id);
            if (!isLeader) {
...
app.post('/api/student/verify-otp', async (req, res) => {
    ...
    try {
        ...
        const { data: college } = await supabase_1.supabaseAdmin
            .from('colleges')
            .select('name')
...
app.get('/api/slots', async (req, res) => {
    try {
        const { data: slots, error: slotsError } = await supabase_1.supabaseAdmin
            .from('slots')
            .select('*')
            .order('id');
        if (slotsError) {
        ...
        const { data: students, error: studentsError } = await supabase_1.supabaseAdmin
            .from('students')
            .select('slot_id');
        if (studentsError) {
            return res.status(500).json({ success: false, error: studentsError.message });
        }
        const slotCounts = (students || []).reduce((acc, curr) => {
            if (curr.slot_id) {
...
app.post('/api/students/:id/select-slot', async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const { data: slotData, error: slotError } = await supabase_1.supabaseAdmin
            .from('slots')
            .select('label')
            .eq('id', slotId)
            .single();
        ...
        const { data, error } = await supabase_1.supabaseAdmin.rpc('assign_student_slot', {
            p_student_id: id,
            p_slot_id: slotId
        });
        if (error) {
        ...
        try {
            const { data: slotStudents } = await supabase_1.supabaseAdmin
                .from('students')
                .select('id, team_id')
                .eq('slot_id', slotId)
                .not('id', 'eq', id);
...
app.get('/api/students', async (req, res) => {
    ...
    try {
        ...
        const withTeams = (data || []).map((s) => {
            const r1ScoreObj = Array.isArray(s.scores) ? s.scores.find((sc) => sc.round === 'round1') : null;
            return {
                ...s,
...
app.post('/api/students/assign-team', async (req, res) => {
    const { studentIds, teamName } = req.body;
    if (!studentIds || !Array.isArray(studentIds)) {
        return res.status(400).json({ success: false, error: 'studentIds array is required' });
    }
    const cleanupEmptyTeams = async (collegeId) => {
        try {
            const { data: teams } = await supabase_1.supabaseAdmin
                .from('teams')
                .select('id, name')
            ...
            if (teams && teams.length > 0) {
                for (const t of teams) {
                    ...
                    if (count === 0 && !t.name.endsWith(' (Individual)')) {
                        await supabase_1.supabaseAdmin.from('teams').delete().eq('id', t.id);
    ...
    try {
        if (!teamName) {
            const { data: studentsData, error: sErr } = await supabase_1.supabaseAdmin
                .from('students')
            ...
            for (const student of studentsData) {
                ...
                if (existingTeam) {
                    ...
                    const { data: newTeam, error: teamErr } = await supabase_1.supabaseAdmin
                        .from('teams')
        ...
        let teamId;
        const { data: teamData } = await supabase_1.supabaseAdmin
            .from('teams')
            .select('id, leader_id')
            .eq('college_id', collegeId)
            .eq('name', trimmedName)
        ...
        if (teamData) {
            ...
            const { data: existingMembers } = await supabase_1.supabaseAdmin
                .from('students')
                .select('slot_id, name')
                .eq('team_id', teamId);
            ...
            if (!teamData.leader_id && studentIds.length > 0) {
                ...
                await supabase_1.supabaseAdmin.from('teams').update({ leader_id: randomLeaderId }).eq('id', teamId);
            }
        }
        else {
            // Create new team, appointing a team leader randomly
            const randomLeaderId = studentIds[Math.floor(Math.random() * studentIds.length)];
            const { data: newTeam, error: tErr } = await supabase_1.supabaseAdmin
                .from('teams')
    ...
    try {
        ...
        if (existingTeam) {
            ...
            if (teamErr || !newTeam) {
                ...
                await supabase_1.supabaseAdmin.from('students').delete().eq('id', userId);
                await supabase_1.supabaseAdmin.auth.admin.deleteUser(userId);
        ...
        if (updateErr) {
            ...
            await supabase_1.supabaseAdmin.from('teams').delete().eq('id', teamId);
            await supabase_1.supabaseAdmin.from('students').delete().eq('id', userId);
            await supabase_1.supabaseAdmin.auth.admin.deleteUser(userId);
...
app.post('/api/students/bulk', async (req, res) => {
    ...
    try {
        ...
        for (const s of students) {
            ...
            try {
                ...
                for (const created of createdStudents) {
                    await supabase_1.supabaseAdmin.auth.admin.deleteUser(created.id);
                }
                return res.status(500).json({ success: false, error: err.message });
            }
        }
        // 2. Insert student records with team_id = null first to satisfy foreign key constraints
        const studentsToInsert = createdStudents.map((s) => ({
            ...s,
            team_id: null
        }));
        const { data: insertedStudents, error: dbError } = await supabase_1.supabaseAdmin
            .from('students')
        ...
        const teamsToCreate = (insertedStudents || []).map((s) => ({
            name: `${s.name} (Individual)`,
            college_id: s.college_id,
            leader_id: s.id
        }));
        const { data: createdTeams, error: teamsError } = await supabase_1.supabaseAdmin
            .from('teams')
        ...
        if (teamsError) {
            ...
            await supabase_1.supabaseAdmin.from('students').delete().in('id', studentIds);
            for (const created of createdStudents) {
                await supabase_1.supabaseAdmin.auth.admin.deleteUser(created.id);
            }
        ...
        for (const team of (createdTeams || [])) {
            ...
            const match = (insertedStudents || []).find((s) => s.id === team.leader_id);
            if (match) {
...
app.put('/api/teams/:id/qualify', async (req, res) => {
    const { id } = req.params;
...
app.post('/api/teams/notify', async (req, res) => {
    ...
    try {
        ...
        const { data: slotStudents, error: studentsError } = await supabase_1.supabaseAdmin
            .from('students')
        ...
        for (const [groupName, members] of Object.entries(groups)) {
            const teamsData = members[0]?.teams;
            ...
            if (leaderId) {
                ...
                if (leader) {
                    ...
                    const { data: leaderData } = await supabase_1.supabaseAdmin
                        .from('students')
                        .select('name')
                        .eq('id', leaderId)
                        .maybeSingle();
          ...
          <td style="padding: 8px 12px; border-bottom: 1px solid #eee; font-size: 14px; color: #555;">${m.phone}</td>
            ...
            for (const member of members) {
              ...
              <div style="font-family: Arial, sans-serif; padding: 24px; border: 1px solid #ddd; max-width: 600px; border-radius: 10px; background: #fff;">
                ...
                <p style="color: #333; font-size: 15px; margin-bottom: 20px;">
                  Hello <strong>${member.name}</strong>, you have been assigned to a group for the MarkUp competition.
                </p>
            ...
            `
                    ...
                    console.error(`Failed to send group email to ${member.email}:`, mailErr);
                    failed++;
                }
            }
        }
...
// COLLEGE SETTINGS
// =====================================
app.get('/api/college-settings/:college_id', async (req, res) => {
...
app.post('/api/college-settings/:college_id', async (req, res) => {
    const { college_id } = req.params;
...
app.post('/api/students/:id/reset-test', async (req, res) => {
    const { id } = req.params;
    try {
        await supabase_1.supabaseAdmin.from('scores').delete().eq('student_id', id);
        const { error } = await supabase_1.supabaseAdmin
            .from('students')
            .update({
            r1_score: null,
...
// SCORE SUBMISSION
// =====================================
app.post('/api/students/:id/submit-score', async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const { data: existingScore, error: checkError } = await supabase_1.supabaseAdmin
            .from('scores')
            .select('id')
            .eq('student_id', id)
            .eq('round', round)
        ...
        const { error: scoreError } = await supabase_1.supabaseAdmin
            .from('scores')
            .insert([{
                student_id: id,
                round,
...
// =====================================
app.post('/api/students/:id/submit-round', async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const { data: student, error: fetchErr } = await supabase_1.supabaseAdmin
            .from('students')
        ...
        const { college_id, team_id, teams } = student;
        if (!team_id) {
        ...
        const { data: settings, error: settingsErr } = await supabase_1.supabaseAdmin
            .from('college_settings')
            .select('*')
            .eq('college_id', college_id)
            .maybeSingle();
        ...
        const { data: teamMembers, error: membersErr } = await supabase_1.supabaseAdmin
            .from('students')
        ...
        const { data: slots, error: slotsErr } = await supabase_1.supabaseAdmin
            .from('slots')
            .select('id, label');
...
// =====================================
app.post('/api/students/:id/jury-review', async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        if (roundKey === 'round2') {
            if (status)
                updateData.round2_status = status;
            if (score !== undefined)
                updateData.r2_score = score;
        }
        else {
            if (status)
                updateData.round3_status = status;
            if (score !== undefined)
                updateData.r3_score = score;
        }
        // 3. Update the whole team if the student is assigned to one, otherwise update individual
        let query = supabase_1.supabaseAdmin.from('students').update(updateData);
        if (team_id) {
